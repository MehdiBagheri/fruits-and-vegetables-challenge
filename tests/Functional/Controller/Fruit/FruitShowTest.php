...
class FruitShowTest extends FunctionalTestCase
{
    ...
    {
        $fruit = $this->createTestFruit('Mango', 400);

        $this->client->request(
            Request::METHOD_GET,
            $this->generateUrl('fruit_show', ['id' => $fruit->getId()])
        );
        $this->assertResponseStatusCodeSame(Response::HTTP_OK);
        $this->assertResponseHeaderSame('content-type', 'application/json');
        $response = json_decode($this->client->getResponse()->getContent(), true);
        $this->assertEquals('Mango', $response['name']);
    ...
    {
        $fruit = $this->createTestFruit('Watermelon', 5000);

        $this->client->request(
            Request::METHOD_GET,
            $this->generateUrl('fruit_show', ['id' => $fruit->getId(), 'unit' => 'kg'])
        );
    ...
    public function shouldRetrieveSingleFruitWithGramUnit(): void
    {
        ...
        $this->client->request(
            Request::METHOD_GET,
            $this->generateUrl('fruit_show', ['id' => $fruit->getId(), 'unit' => 'g'])
        );
    ...
    {
        $this->client->request(Request::METHOD_GET, $this->generateUrl('fruit_show', ['id' => 9999]));
